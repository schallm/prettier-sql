...
- **Always Encrypted** — `CREATE/DROP COLUMN MASTER KEY`, `CREATE/ALTER/DROP COLUMN ENCRYPTION KEY` (ADD/DROP VALUE, multiple key-rotation tuples), and the column-level `ENCRYPTED WITH (COLUMN_ENCRYPTION_KEY = ..., ENCRYPTION_TYPE = ..., ALGORITHM = ...)` clause

**AI / External Models** (SQL Server 2025)

- `CREATE/ALTER/DROP EXTERNAL MODEL` — `WITH`/`SET` options (`LOCATION`, `API_FORMAT`, `MODEL_TYPE`, `MODEL`, `CREDENTIAL`, `PARAMETERS`, `LOCAL_RUNTIME_PATH`), optional `AUTHORIZATION` on `CREATE`
