/**
 * Integration tests for bundled/format.mjs.
...
const FORMAT_SCRIPT = join(__dirname, '..', 'bundled', 'format.mjs');

function runFormat(sql, optionsJson = null, filePath = null) {
    const args = [FORMAT_SCRIPT];
...
test('formats a basic SELECT statement', () => {
    const result = runFormat('select id,name from users');
    assert.equal(result.status, 0, result.stderr);
    assert.ok(result.stdout.length > 0);
});

test('sqlKeywordCase upper produces uppercase keywords', () => {
    const result = runFormat('select id from users', JSON.stringify({ sqlKeywordCase: 'upper' }));
    assert.equal(result.status, 0, result.stderr);
...
test('sqlKeywordCase lower produces lowercase keywords', () => {
    const result = runFormat('SELECT ID FROM USERS', JSON.stringify({ sqlKeywordCase: 'lower' }));
    assert.equal(result.status, 0, result.stderr);
    assert.match(result.stdout, /select/);
    assert.match(result.stdout, /from/);
});
...
test('respects .prettierrc config in the file directory', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'prettier-tsql-test-'));
    try {
        writeFileSync(join(tmpDir, '.prettierrc'), JSON.stringify({ sqlKeywordCase: 'upper' }));
        const result = runFormat('select id from users', null, join(tmpDir, 'query.sql'));
        assert.equal(result.status, 0, result.stderr);
        assert.match(result.stdout, /SELECT/);
    } finally {
...
test('explicit options override .prettierrc', () => {
    ...
    try {
        writeFileSync(join(tmpDir, '.prettierrc'), JSON.stringify({ sqlKeywordCase: 'upper' }));
        // Explicit option lower should win over config upper
...
test('invalid options JSON exits with code 2', () => {
    const result = runFormat('select 1', 'not-valid-json');
